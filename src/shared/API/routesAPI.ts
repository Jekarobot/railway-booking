...
export const useRoutesAPI = () => {
  ...
  const buildQueryString = (params: Routes) => {
    ...
    if (params.have_first_class) queryParts.push(`have_first_class=${params.have_first_class}`)
    if (params.have_second_class) queryParts.push(`have_second_class=${params.have_second_class}`)
    if (params.have_third_class) queryParts.push(`have_third_class=${params.have_third_class}`)
    if (params.have_fourth_class) queryParts.push(`have_fourth_class=${params.have_fourth_class}`)
    if (params.have_wifi) queryParts.push(`have_wifi=${params.have_wifi}`)
    if (params.have_air_conditioning)
      queryParts.push(`have_air_conditioning=${params.have_air_conditioning}`)
    if (params.have_express) queryParts.push(`have_express=${params.have_express}`)
    if (params.price_from !== undefined && params.price_from !== 0)
      queryParts.push(`price_from=${params.price_from}`)
